document.addEventListener("DOMContentLoaded", () => {
    ...
    document.getElementById("loadComplaintsBtn")
        ...
        const response = await fetch(
            "http://127.0.0.1:8000/api/police/complaints/all/",
            {
                headers: {
    ...
    document.getElementById("loadSOSBtn")
        ...
        const response = await fetch(
            "http://127.0.0.1:8000/api/sos/all/",
            {
                headers: {
                    "Authorization": "Bearer " + token
                }
    ...
    complaints.forEach(c => {
        html += `
            <tr data-id="${c.id}">
                ...
                <td>${c.created_at || "-"}</td>
                <td>
                    <span class="status-badge status-${c.status} status-cell">
                        ${c.status}
                    </span>
                </td>
                <td>
...
function renderSOS(sosList) {
    ...
    ? `
        <video width="200" controls>
            <source src="http://127.0.0.1:8000${s.video}" type="video/mp4">
    ...
    : "-";
        html += `
            <tr data-id="${s.id}">
                ...
                <td>${s.created_at || "-"}</td>
                <td>
                    <span class="status-badge status-${s.status} status-cell">
                        ${s.status}
                    </span>
                </td>
                <td>${videoLink}</td>
...
async function updateComplaint(id, status) {
    ...
    const response = await fetch(
        "http://127.0.0.1:8000/api/police/complaints/update-status/",
        {
            method: "POST",
            headers: {
    ...
    if (response.ok) {
        ...
        badge.className = `status-badge status-${status} status-cell`;
...
async function updateSOS(id, status) {
    ...
    const response = await fetch(
        "http://127.0.0.1:8000/api/sos/update-status/",
        {
            method: "POST",
            headers: {
                "Content-Type": "application/json",
